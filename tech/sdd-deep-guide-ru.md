...
> "Формальная, машиночитаемая спецификация служит авторитетным источником истины и основным артефактом, из которого выводятся реализация, тестирование и документация."
...
> "Продуманная спецификация пишется сначала, а затем используется в AI-assisted workflow."
...
> "Спецификация сохраняется даже после завершения задачи, чтобы продолжать использоваться для развития и поддержки."
...
> "Спецификация — основной source-файл со временем, и только спеку редактирует человек, человек никогда не трогает код."
...
- UML модель = spec
- Кодогенератор → код
- Проблемы: inflexibility, overhead, tool lock-in
...
> "Даже со всеми этими файлами, шаблонами, промптами, воркфлоу и чеклистами, я часто видел как агент в итоге не следовал всем инструкциям."
...
**Важно:** SDD не заменяет программирование — это **фреймворк** для работы с AI-агентами. Опытный 1С-разработчик понимает когда нужен контроль, а когда можно довериться AI.
